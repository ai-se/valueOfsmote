...
\lstset{
    ...
    emph={progressive, build_model, rank_based, projective, measure, m, r, k, frontier, cf, f, g}, emphstyle=\bfseries\color{blue!50!black},
...
% \be
% \item A discovery of systematic bias in  prior ranking studies.
% \item A novel application of search-based SE.
% \item Better defect predictors.
...
% \item A reproduction package to reproduce our results then (perhaps) to improve or refute  our results\footnote{Available from download from http://XXXblinded4review}.
% \item Novel challenge problems to guide future research in 
% software analytics:
% \be
% \item
% Are there other treatments for data that result in larger performance gains than those seen here?
% \item
% For software analytics,  is it better to improve our data miners, or our data pre-processors, or both?
% \ee
% \ee
...
\end{tabular}
\end{center}
\caption{Dataset statistics. Datasets are sorted from low percentage of defective class to high defective class. Note that when data was available for multiple
versions of the same software, we used the latest one.
Data comes from the SEACRAFT repository: http://tiny.cc/seacfraft}.}
\label{tb:dataset}
\end{table}

\subsection{Within- vs Cross-Measure Assessment}
We call the above rig the {\em within-measure assessment rig} since it is   biased in its evaluation measures. Specifically,  in this rig,
when SMOTUNED in optimizing for (e.g.) AUC, we do not explore the effects on (e.g.) the false alarm. This is less than ideal
since it is know that our performance measures are inter-connected via the Zhang equation~\cite{zhang2007comments}. Hence, increasing (e.g.) recall might potentially have the adverse
...
In summary, the precisions and false alarm rates
do not negate the value of SMOTUNED while the AUC and recall results are strongly
positive about  use of SMOTUNED.
...
% \item Whenever we want to maximise a certain evaluation goal, specifically AUC and recall, then it is recommended to tune the parameters of SMOTE.
