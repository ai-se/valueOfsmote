...
in the frequency of the defect class. Especially, we use data sets where the percent
...
In this context, we explore four research questions:  
 ...
 \begin{lesson}DE finds different ``best'' parameter settings for SMOTE for different data sets. Hence reusing tunings  suggested  by  any other  previous study  for any data set is \underline{{\em not}} recommended. Instead,  it is better to
 ...
 \begin{lesson}Tuning with DE makes training four to five times slower, but the improvements which we get for AUC and recall is quite advantageous.
 \end{lesson}
...
This experiment3  performed a 3 times 5 stratified cross validation~\cite{refaeilzadeh2009cross}:
\bi
\item Three times we randomized the order of the training set;
...
As per Ghotra et al.~\cite{ghotra2015revisiting}, we use k=8 for k Nearest Neighbours. Also for Decision trees and Random Forest we are using Entropy as split criteria. Most of these implementations are provided in Scikit-Learn~\cite{pedregosa2011scikit} and available open source.
...
the experiment 15 times by randomly changing the order of the data samples each time.

\section{Conclusion}
\label{sect:conclusion}

We were able to reproduce the baseline paper "Revisiting the impact of classification techniques on the performance of defect prediction models" with the same conclusions what Ghotra et al. found. But for class imbalance problem, it is concluded to get better AUC and recall, SMOTE~1 needs to be performed. The improvements in AUC and recall with SMOTE~2 is dramatic. Tuning is important if we have a specific tuning goal like maximizing AUC or recall. When comparing the run times against SMOTE~1, SMOTE~2 is about four to five times slower.
