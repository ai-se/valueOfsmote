...
One way to handle imbalance is using the SMOTE procedure (Synthetic Minority Oversampling Technique) which 
...
of SMOTE, referred to as SMOTE2,
...
called Differential Evolution (DE). 
 ...
 9 defect datasets, we find that SMOTE1 may or may not be useful depending on the evaluation goal. We observed mixed results for precision (sometimes better sometimes worse).
On the other hand, the tunings
used in SMOTE2 lead to dramatic improvements in  AUC (pf, recall) and modest improvements were seen in recall and these were much better than SMOTE1.
...
a software system with a higher probable quantity of defects. Current ongoing work in defect prediction focuses on (i) estimating the number of defects remaining in software systems, (ii) discovering defect associations, and (iii) classifying the defect-proneness of software components, typically into two classes defect-prone and not defect-prone. 
...
since most software systems have less percentage of defective classes. Accordingly, this paper
...
We refer this as tuned version SMOTE2.
...
explore  the following  questions:  
 ...
 \textbf{RQ3}: \textbf{Do different datasets
 ...
 \end{lesson}
  ...
  since it means
  reusing tunings  suggested  by  any other  previous study  for any dataset is \underline{{\em not}} recommended. Instead,  it is better to
 ...
 \begin{lesson}Tuning with DE makes training runtimes four to five times slower, but given
...
There are other added advantages that come with CK metrics is they are  simple  to  compute. CK metrics is endorsed by much of the community since it has been used in past covering research papers about twice (49\%) 
...
and build models on the latent feature space. Too many variables in the data
...
{\em majority} class. In this paper, we say that imbalance is worse when the frequency of
the minority class {\em decreases}, that is,
{\em minority=5\%} is worse than {\em minority=20\%}.

Misclassifying an example from the minority class is usually more expensive. For software defect prediction, due to the nature of the problem, the defect case is much less likely to happen than the non-defect case.  The failure of finding a defect could degrade software quality greatly (since more bugs
...
imbalance problems at data and algorithm levels. Data-level method manipulate the training data to select some or whole for training. This
...
performance of C4.5 is unstable on imbalanced datasets. They studied the stability issues due to  class imbalance and found out that Random Forest and Naive Bayes are the most stable. They generated synthetic datasets from the original one to build class imbalance datasets. This could be affected by random sampling so this may not be the ideal way to build imbalanced datasets. They also used default set of parameters but wanted to study the effects of tuning classifiers as well as mentioned to use techniques like SMOTE for class imbalance in their future work.

Wang et al.~\cite{wang2013using} studied various undersampling and oversampling technique and compared the results with Naive Bayes and random forest. Other interesting finding from Wang was to use AdaBoost.NC which has better performance than the rest while others like Gray~\cite{gray2009using} is planning to use SMOTE in future studies. Yan et al.~\cite{yan2010software} performed fuzzy logic and rules to overcome the imbalance problem only to work with Support Vector Machines. 
...
chosen. To find nearest neighbors different distance metric is used. Most common distance metric Minkowski~\cite{deza2009encyclopedia} which has power ($r$) in its formula. Our implementation currently uses 5 nearest neighbors for SMOTE1 method and for SMOTE2, DE finds the best $k$. Synthetic samples
...
Past researchers have mentioned many optimizers like: simulated
...
We have 3 techniques which we have named as, (a) Baseline learner results without SMOTE (No-SMOTE), (b) Standard SMOTE with default parameters (SMOTE1) and (c) Tuned SMOTE using DE (SMOTE2). To reduce the variation, each of the above techniques is performed with a 5-fold stratified cross validation~\cite{refaeilzadeh2009cross} which means:
\bi
...
\bi
 ...
 $k$-Nearest Neighbors algorithm (or $k$-NN for short) is a non-parametric method used for classification and regression. In both cases, the input consists of the $k$ closest training examples in the feature space. Since $k$ is even, for tie breaking a random result is selected.
